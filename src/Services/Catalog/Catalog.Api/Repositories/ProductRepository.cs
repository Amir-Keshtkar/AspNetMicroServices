...
namespace Catalog.Api.Repositories {
    public class ProductRepository: IProductRepository {
        ...
        public async Task CreateProduct(Product command) {
            await _catalogContext.Products.InsertOneAsync(command);
        }

        public async Task<bool> UpdateProduct(Product command) {
            var result = await _catalogContext.Products.ReplaceOneAsync(x => x.Id == command.Id, command);
